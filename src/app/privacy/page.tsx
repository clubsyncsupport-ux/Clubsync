          ...
          <main>
            ...
            <div className="legal-part">
              ...
              <section className="legal-clause" id="p6">
                ...
                <p>Your account and activity data — the actual database — is hosted with Supabase, in Supabase&rsquo;s Canada (Central) data centre. Your information is encrypted in transit (HTTPS) and the database itself is not publicly accessible.</p>
                <p>Two other services touch a narrower slice of your information as part of running the site, and aren&rsquo;t confirmed to store or process data in Canada the way the database is: ClubSync&rsquo;s website itself runs on Vercel, whose servers primarily operate in the United States, and password-reset emails are sent through Resend, a transactional email provider that stores email metadata in the United States. Neither one receives your broader account data — Vercel only handles a request as it happens, and Resend only ever sees a one-time reset link and the email address for that single message — but we&rsquo;d rather say this plainly than let &ldquo;the database is in Canada&rdquo; be read as &ldquo;everything about ClubSync is in Canada,&rdquo; which isn&rsquo;t accurate yet.</p>
                <div className="legal-note">
                  <strong>If your school formally adopts ClubSync</strong> as an official tool (rather than the current independent, student-run pilot), British Columbia&rsquo;s public-sector privacy law (FIPPA) still expects a Privacy Impact Assessment as part of that formal adoption process, which would look closely at exactly this — where every part of the system actually runs, not just the database. That review would happen as part of the school taking ClubSync on, not something silently skipped.
                </div>
              </section>
              ...
              <section className="legal-clause" id="p8">
                ...
                <p>We keep your account and activity data for as long as your account is active. You can delete your account and personal information yourself at any time from Settings, and it&rsquo;s removed right away — your email, password, sign-in access, profile photo, club memberships, event registrations, notifications, and any unverified service hours are all gone immediately and permanently.</p>
                <p>The one exception is <em>verified</em> service hours: since a school may need those totals as part of its own records, deleting your account keeps your name attached to just those specific verified records, even though the account itself can never be signed into again — everything that would let you (or anyone else) use, access, or be contacted through that account is still removed. How long the school itself needs to keep that record afterward is up to the school&rsquo;s own retention requirements, not something we set unilaterally.</p>
